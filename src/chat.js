 ...
 */
window.ET_Chat.onload = function(config) {
  if (!config) {
   ...
   */
  function userOnlineWrapper() {
    chatObj.userOnline();
  }

  setTimeout(userOnlineWrapper, 1000);
  if (Prototype.Browser.IE) {
    $("splitpane").onmousedown = function() {
...
function ET_Chat(config) {
  ...
  this.user_id = ""; //(public)
  this.set_dynamic_height = true; //(public) sonst hat das ganze Chat eine feste Hoehe
  ...
  // nachdem oben die Defaults gesetzt wurden, nun die Werte aus der Config
  // ... jaa - das ginge auch in einem Schritt.
  this.messages_im_chat = config.messages_im_chat
    ? config.messages_im_chat
    : this.messages_im_chat;
  this.reload_interval = config.reload_interval
    ? config.reload_interval
    : this.reload_interval;
  this.username = config.username ? config.username : this.username;
  this.user_id = config.user_id ? config.user_id : this.user_id;
  this.userPrivilegienGlobal = config.userPrivilegienGlobal
    ? config.userPrivilegienGlobal
    : this.userPrivilegienGlobal;
  this.win_style = config.win_style ? config.win_style : this.win_style;
  this.anbindung_an_userverwaltung = config.anbindung_an_userverwaltung
    ? config.anbindung_an_userverwaltung
    : this.anbindung_an_userverwaltung;
  this.show_history_all_user = config.show_history_all_user
    ? config.show_history_all_user
    : this.show_history_all_user;
  this.allowed_privates_in_chat_win = config.allowed_privates_in_chat_win
    ? config.allowed_privates_in_chat_win
    : this.allowed_privates_in_chat_win;
  this.allowed_privates_in_separate_win = config.allowed_privates_in_separate_win
    ? config.allowed_privates_in_separate_win
    : this.allowed_privates_in_separate_win;
  this.interval_for_inactivity = config.interval_for_inactivity
    ? config.interval_for_inactivity
    : this.interval_for_inactivity;
  this.set_dynamic_height = config.set_dynamic_height
    ? config.set_dynamic_height
    : this.set_dynamic_height;
  this.allow_nick_registration = config.allow_nick_registration
    ? config.allow_nick_registration
    : this.allow_nick_registration;
  this.textcolor = config.textcolor ? config.textcolor : this.textcolor;
  this.soundManager = config.soundManager
    ? config.soundManager
    : this.soundManager;
  this.sound_status = config.sound_status
    ? config.sound_status
    : this.sound_status;
  this.set_sys_messages = config.set_sys_messages
    ? config.set_sys_messages
    : this.set_sys_messages;
  this.random_user_number = config.random_user_number
    ? config.random_user_number
    : this.random_user_number;
  ...
  this.start = function() {
    if (Prototype.Browser.IE) {
      document.onfocusin = function(e) {
        ...
        try {
          window.clearInterval(self.intv_title_blink);
        } catch (e) {
          /*nix*/
        }
    ...
    if (self.userPrivilegienGlobal == "admin") {
      $("form_right").innerHTML +=
        '&nbsp;&nbsp;&nbsp;<img id="link_admin" class="img_button" src="img/admin.png" width="32" height="32" border="0" alt="Admin" title="Admin">';
      $("link_admin").onclick = function() {
        var hoehe = $("chatinhalt").getHeight();
        var breite = $("chatinhalt").getWidth();
        var win_admin = new Window({
          url: "./?AdminIndex",
    ...
    $("message_form").onsubmit = function() {
      return self.send();
    }; // Wichtig damit beim Submit das Dokument nicht neu geladen wird.
    $("link_sagen").onclick = function() {
      return self.send();
    }; // s.o. Zeile.
    ...
    $("link_prop").onclick = function() {
      ...
      if (typeof self.win_prop != "object") {
        // Fensterinstanz
        self.win_prop = new Window({
          className: self.win_style,
          width: 260,
          height: 170,
          top: eval(self.mouse_top - 235),
          left: eval(self.mouse_left - 120),
          resizable: false,
          showEffect: Effect.Appear,
          hideEffect: Effect.Fade,
          showEffectOptions: { duration: 0.5 },
        ...
        // History anzeigen?
        if (
          self.show_history_all_user ||
          self.userPrivilegienGlobal == "admin" ||
          self.userPrivilegienGlobal == "mod"
        )
          var history_link_content =
            '<div style="margin-bottom:3px;"><img src="img/time.png" align="left" />&nbsp;&nbsp;<a href="#" id="history">' +
            lang_start_prop_link_2 +
            "</a></div>";
        else var history_link_content = "";

        // PW aenderung fuer Mods und User anzeigen?
        if (
          self.userPrivilegienGlobal == "mod" ||
          self.userPrivilegienGlobal == "user"
        ) {
          var pwchange_link_content =
            '<div style="margin-bottom:3px;"><img src="img/textfield_key.png" align="left" />&nbsp;&nbsp;<a href="#" id="pwchange_mod">' +
            lang_start_prop_link_6 +
            '</a>\
                      <form id="pwchange_form" style="display:inline;"><div id="pwchange_div" style="display:none;"><div style="padding-top: 2px;">' +
            lang_start_prop_link_7 +
            ' <input type="password" id="pwchange_field" size="6" /> <a href="#" id="pwchange_mod_ok">OK</a></div></div></form></div>';
          if (self.userPrivilegienGlobal == "user")
            pwchange_link_content +=
              '<div style="margin-bottom:3px;"><img src="img/key_delete.png" align="left" />&nbsp;&nbsp;<a href="#" id="unregister_name">' +
              lang_start_prop_link_10 +
              "</a></div>";
        } else if (
          self.userPrivilegienGlobal == "gast" &&
          self.allow_nick_registration
        )
          var pwchange_link_content =
            '<div style="margin-bottom:3px;"><img src="img/textfield_key.png" align="left" />&nbsp;&nbsp;<a href="#" id="register_name">' +
            lang_start_reg_link +
            "</a></div>";
        else var pwchange_link_content = "";

        // Status Auswahl generieren
        var status_link_content =
          '<div><img src="img/status_online.png" align="left" />&nbsp;&nbsp;<a href="#" id="stat_mod">' +
          lang_statuslink +
          '</a>\
        ...
        for (var i = 0; i < lang_status_text.length; i++) {
          if (lang_status_rights[i] == "all")
            status_link_content +=
              '<img src="img/' +
              lang_status_imgname[i] +
              '.png"/> <a href="#" id="' +
              lang_status_imgname[i] +
              '">' +
              lang_status_text[i] +
        ...
        status_link_content +=
          '</div></div><div style="margin-bottom:3px;"></div>';

        var sys_mess_checked = self.set_sys_messages ? "checked" : "";

        // Fenster fuellen
        self.win_prop.setHTMLContent(
          '<div id="prop_list"><div style="margin-bottom:3px;"><img src="img/monitor_lightning.png" align="left" />&nbsp;&nbsp;<a href="#" id="cls">' +
      ...
      " +
            status_link_content +
            '\
      <div style="padding-bottom:5px;"><form id="set_sys_messages_form" style="display:inline;"><input type="checkbox" id="set_sys_messages" value="1" ' +
            sys_mess_checked +
            ' /> <a href="#" id="set_sys_messages_a"> ' +
            lang_start_prop_link_9 +
            '</a></form></div>\
      <div style="margin-bottom:1px;"><img src="img/sound_' +
            self.sound_status +
            '.png" align="left" id="sound_icon" />&nbsp;&nbsp;' +
            lang_start_prop_link_3 +
            '</div>\
      <div><img src="img/space.gif" align="left" width="16"/>&nbsp;&nbsp;<a href="#" id="sound_on">' +
            lang_start_prop_link_4 +
            '</a> | <a href="#" id="sound_privat">' +
            lang_start_prop_link_8 +
            '</a> | <a href="#" id="sound_off">' +
            lang_start_prop_link_5 +
            "</a></div></div>\
      "
        );
        ...
        Event.observe("prop_list", "click", function(event) {
          if (
            ...
            Event.element(event).id != "prop_list" &&
            Event.element(event).id != "sound_icon"
          ) {
            if (Event.element(event).id == "cls") {
              $("chatinhalt").innerHTML = "";
              self.win_prop.close();
              $("message").focus();
            }
            if (Event.element(event).id == "pwchange_mod_ok") submit_pw();
            if (Event.element(event).id == "history") {
              self.historyWindow(1);
              self.win_prop.close();
            }
            if (Event.element(event).id == "stat_mod")
              Effect.toggle("stat_change_div", "blind", { duration: 0.4 });
            if (
              Event.element(event).id == "set_sys_messages" ||
              Event.element(event).id == "set_sys_messages_a"
            ) {
              if (Event.element(event).id == "set_sys_messages_a") {
                if ($("set_sys_messages").checked)
                  $("set_sys_messages").checked = false;
                else $("set_sys_messages").checked = true;
              }
              var sys_mess = $("set_sys_messages").checked ? 1 : 0;
              new Ajax.Request("./?ChangeStatus", {
                postBody: "sys_messages=" + sys_mess
              });
              self.win_prop.close();
              $("message").focus();
            }
            if (Event.element(event).id.slice(0, 7) == "status_")
              submit_status(
                Event.element(event).id,
                $(Event.element(event).id).innerHTML
              );

            if (Event.element(event).id == "pwchange_mod") {
              Effect.toggle("pwchange_div", "blind", { duration: 0.4 });
              $("pwchange_form").onsubmit = function() {
                return submit_pw();
              };
            }
            ...
            if (Event.element(event).id == "register_name") {
              var temp_user_name =
                self.username.length > 20
                  ? self.username.slice(0, 20) + "..."
                  : self.username;

              var win_register_user = new Window({
                className: self.win_style,
                title: lang_start_reg_title + ' "' + temp_user_name + '"',
                width: 250,
                height: 130,
                top: eval(self.mouse_top - 185),
                left: eval(self.mouse_left - 120),
                resizable: false,
                showEffect: Effect.Appear,
                hideEffect: Effect.Fade,
              ...
              win_register_user.setHTMLContent(
                '<div id="register_formular"><form id="register_form" style="display:inline;"><div>' +
                  lang_start_reg_befor_registering +
                  '<hr size="1"><table><tr><td>' +
                  lang_start_reg_pw1 +
                  '</td><td><input type="password" id="pw_register_field" size="6" /></td></tr><tr><td>' +
                  lang_start_reg_pw2 +
                  '</td><td><input type="password" id="pw_register_field2" size="6" /></td></tr><tr><td colspan="2"><br /><a href="#" id="make_register">' +
                  lang_start_reg_button_register +
                  '</a>&nbsp;&nbsp;&nbsp;<a href="#" id="cancel_register">' +
                  lang_start_reg_button_cancel +
                  "</a></td></tr></div></form></div>"
              );
              Event.observe("register_formular", "click", function(event) {
                if (Event.element(event).id == "cancel_register")
                  win_register_user.close();
                        ...
                        if (result.responseText == 1) {
                          win_register_user.setHTMLContent(
                            '<div id="register_formular">' +
                              lang_start_reg_after_registering +
                              '<br /><br /><a href="./?Logout&random_user_number=' +
                              self.random_user_number +
                              "&r=" +
                              $("room").value +
                              '">' +
                              lang_start_reg_after_registering_link +
                              "</a></div>"
                          );
                        } else alert("Error!\n\n" + result.responseText);
                      },
            ...
            if (Event.element(event).id == "unregister_name") {
              ...
              var win_unregister_user = new Window({
                className: self.win_style,
                title: lang_remove_pw_win_title + ' "' + temp_user_name + '"',
                width: 320,
                height: 180,
                top: eval(self.mouse_top - 225),
                left: eval(self.mouse_left - 170),
                resizable: false,
                showEffect: Effect.Appear,
                hideEffect: Effect.Fade,
                showEffectOptions: {
              ...
              win_unregister_user.setHTMLContent(
                '<div id="unregister_formular"><form id="unregister_form" style="display:inline;"><div>' +
                  lang_remove_pw_win_text_1 +
                  "<br><br><b>" +
                  lang_remove_pw_win_text_2 +
                  "</b><br>" +
                  lang_remove_pw_win_text_3 +
                  '<hr size="1"><table><tr><td>' +
                  lang_start_reg_pw1 +
                  '</td><td><input type="password" id="pw_unregister_field" size="9" /></td></tr><tr><td colspan="2"><br /><a href="#" id="make_unregister">' +
                  lang_remove_pw_win_button +
                  '</a>&nbsp;&nbsp;&nbsp;<a href="#" id="cancel_unregister">' +
                  lang_start_reg_button_cancel +
                  "</a></td></tr></div></form></div>"
              );
              Event.observe("unregister_formular", "click", function(event) {
                if (Event.element(event).id == "cancel_unregister")
                  win_unregister_user.close();
                if (Event.element(event).id == "make_unregister") {
                  if (!$("pw_unregister_field").value.empty()) {
                    var pw_unregister = $("pw_unregister_field").value;
                    $("unregister_formular").update(
                      '<img src="img/ajax-loader.gif">'
                    );
                    new Ajax.Request("./?UnregisterPw", {
                      onSuccess: function(result) {
                        $("unregister_formular").update("Logout...");
                        if (result.responseText == 1) {
                          location.href =
                            "./?Logout&random_user_number=" +
                            self.random_user_number +
                            "&r=" +
                            $("room").value;
                        } else
              ...
              $("unregister_form").onsubmit = function() {
                return false;
              };
            }

            if (Event.element(event).id == "sound_off") {
              self.sound_status = "none";
              $("sound_icon").src = "img/sound_none.png";
              self.win_prop.close();
              $("message").focus();
            }
            if (Event.element(event).id == "sound_on") {
              self.sound_status = "all";
              $("sound_icon").src = "img/sound_all.png";
              self.win_prop.close();
              $("message").focus();
            }
            if (Event.element(event).id == "sound_privat") {
              self.sound_status = "privat";
              $("sound_icon").src = "img/sound_privat.png";
              self.win_prop.close();
              $("message").focus();
            }
          }
    ...
    Event.observe("onlinelist", "click", function(event) {
      if (Event.element(event).id.slice(0, 10) != "infoblock_")
        close_info_win();
    ...
    Event.observe("chatinhalt", "click", function(event) {
      if (Event.element(event).id.slice(0, 10) != "inflblock_")
        close_info_win();
      // send Private Messages and Change Rooms
      changeUserEvent(Event.element(event).id);
    });
    ...
    new Ajax.Request("./?Smileys", {
      onSuccess: function(result) {
        self.win_smileys_content = result.responseText;
      }
    });
    //Click auf Smiley-Icon
    $("link_smileys").onclick = function() {
      open_close_smileys_win("message");
    };
    ...
    $("link_color").onclick = function() {
      ...
      if (typeof self.win_color != "object") {
        self.win_color = new Window({
          className: self.win_style,
          width: 350,
          height: 205,
          top: eval(self.mouse_top - 265),
          left: eval(self.mouse_left - 180),
          resizable: false,
          showEffect: Effect.Appear,
          hideEffect: Effect.Fade,
          showEffectOptions: { duration: 0.5 },
        ...
        $("kursiv").onclick = function() {
          ...
          $("message").style.fontStyle = $("kursiv").checked
            ? "italic"
            : "normal";
        };
  ...
  // (Stop) Konstruktor der Class ET_Chat wird indirekt onLoad ausgefuehrt --------------------------------------
  // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  var open_close_smileys_win = function(input_field_name) {
    ...
    if (typeof self.win_smileys != "object") {
      // Fensterinstanz
      self.win_smileys = new Window({
        className: self.win_style,
        width: 210,
        height: 100,
        top: eval(self.mouse_top - 165),
        left: eval(self.mouse_left - 120),
        resizable: false,
        showEffect: Effect.Appear,
        hideEffect: Effect.Fade,
      ...
      self.win_smileys.setHTMLContent(
        '<div id="smileys_list">' + self.win_smileys_content + "</div>"
      );
    } else {
      if ($(self.win_smileys.getId()).style.display == "none")
        self.win_smileys.setLocation(
          eval(self.mouse_top - 165),
          eval(self.mouse_left - 120)
        );
    }

    try {
      Event.stopObserving("smileys_list", "click");
    } catch (e) {}
    ...
    Event.observe("smileys_list", "click", function(event) {
      if (
        Event.element(event).id != "" &&
        Event.element(event).id != "smileys_list"
      ) {
  ...
  var close_info_win = function(dont_close_id) {
    ...
    self.win_block_ids.each(function(uid) {
      if (
        $(self.win_block[uid].getId()).visible() &&
        self.win_block[uid].getId() != dont_close_id
      )
        self.win_block[uid].close();
    });
  };
  ...
  var window_height = function() {
    ...
    if (window.innerHeight) {
      ...
      if (
        $("chatinhalt").style.height !=
        eval(window.innerHeight - 150) + "px"
      ) {
      ...
      try {
        if (
          $("chatinhalt").style.height !=
          eval(document.documentElement.clientHeight - 150) + "px"
        ) {
          $("chatinhalt").style.height =
            eval(document.documentElement.clientHeight - 150) + "px";
          $("onlinelist").style.height =
            eval(document.documentElement.clientHeight - 150) + "px";
  ...
  var receiveResultJSON = function(ajaxResultJSON) {
    ...
    if (jsonInhalt != "") {
      ...
      for (var i = 0; i < jsonObj.data.length; i++) {
        with (jsonObj.data[i]) {
          ...
          try {
            ...
            console.log("chat.js::receiveResultJSON error: '" + e + "'");
          }

          // Das oben angelegte DIV mit Inhalt auf dem JSON-Object befuellen
          if (sex == "m")
            var gender_icon =
              '<img src="img/user_' + sex + '.png" align="absbottom" />';
          if (sex == "f")
            var gender_icon =
              '<img src="img/user_' + sex + '.png" align="absbottom" />';
          if (sex == "n")
          ...
          if (priv == "admin")
            var priv_icon =
              '<img src="img/' +
              priv +
              '_i_small.png" align="absbottom" title="' +
              priv +
              '" alt="' +
              priv +
              '" />';
          ...
          if (privat == "0") {
            if (user_id == self.user_id) var userstyle_clickable = "";
              ...
              var userstyle_clickable =
                'id="inflblock_' + user_id + '" style="cursor:pointer"';

            $("a" + id).innerHTML =
              '<div class="mess_back"><i>(' +
              time +
              ")</i> " +
              gender_icon +
              priv_icon +
            ...
            if (!id.empty()) {
              ...
              if (
                typeof self.win_private[win_id1[count_priv_win]] != "object" &&
                typeof self.win_private[win_id2] != "object"
              ) {
                //window:kskdskd
                if (user_id == self.user_id) {
                  var privat_win_opponent = self.privat_an;
                  var privat_win_opponent_id = privat;
                } else {
                  var privat_win_opponent = user;
                  var privat_win_opponent_id = user_id;
                }

                self.win_private[win_id1[count_priv_win]] = new Window({
                  className: self.win_style,
                  title: "Privat mit " + privat_win_opponent,
                  width: 380,
                  height: 200,
                  top: eval(50 + Math.round(Math.random() * 50)),
                  left: eval(50 + Math.round(Math.random() * 50)),
                  resizable: true,
                  showEffect: Effect.Appear,
                  hideEffect: Effect.Fade,
                  showEffectOptions: {
                ...
                self.win_private[win_id1[count_priv_win]].setHTMLContent(
                  '<div id="pivate_win_' +
                    win_id1[count_priv_win] +
                    '" class="privat_mesages_window"></div><div><form style="display:inline" id="win_form_' +
                    win_id1[count_priv_win] +
                    '"><table cellspacing="0" cellpadding="0"><tr><td><input type="hidden" id="this_win_' +
                    self.win_private[win_id1[count_priv_win]].getId() +
                    '" value="' +
                    win_id1[count_priv_win] +
                    '"><input type="text" class="private_message_field" id="message_win_' +
                    win_id1[count_priv_win] +
                    '" ></td><td><img src="img/Checked_small.png" id="submit_img_' +
                    win_id1[count_priv_win] +
                    '" style="padding-left: 4px; cursor:pointer;"></td><td><img src="img/Smiley_small.png" id="smileys_img_' +
                    win_id1[count_priv_win] +
                    '" style="padding-left: 4px; cursor:pointer;"></td></tr></table></form></div></div>'
                );
                ...
                myObserver = {
                  onEndResize: function(eventName, win) {
                    ...
                    $(
                      "pivate_win_" + $F("this_win_" + win.getId())
                    ).style.height = eval(win.getSize().height - 33) + "px";
                  },
                ...
                $("win_form_" + win_id1[count_priv_win]).onsubmit = function(
                  e
                ) {
                ...
                $("submit_img_" + win_id1[count_priv_win]).onclick = function(
                  e
                ) {
                ...
                $("smileys_img_" + win_id1[count_priv_win]).onclick = function(
                  e
                ) {
                  var tid = this.id.replace("smileys_img_", "message_win_");
                  open_close_smileys_win(tid);
                };
                $("message_win_" + win_id1[count_priv_win]).onfocus = function(
                  e
                ) {
          ...
          if (user_id != self.user_id) {
            ...
            if (self.sound_status == "none") play_sound = false;
            ...
            if (!self.window_focused && play_sound) {
              ...
              self.intv_title_blink = window.setInterval(function() {
                document.title =
                  document.title == lang_titleAlert
                    ? self.title
                    : lang_titleAlert;
              }, 1000);
            }
      ...
      if (play_sound) {
        //Sound.play('sound/sound.mid',{replace:false});
        ...
        if (div_id > 0) {
          Effect.SwitchOff("a" + div_id, {
            ...
            afterFinish: function(effect) {
              ...
              try {
                if (
                  $("a" + eval(div_id - jsonObj.data.length)).style.display !=
                  "none"
                )
                  $("a" + eval(div_id - jsonObj.data.length)).style.display =
                    "none";
                if (
                  $("a" + eval(div_id - jsonObj.data.length - 1)).style
                    .display != "none"
                )
                  $(
                    "a" + eval(div_id - jsonObj.data.length - 1)
                  ).style.display = "none";
                if (
                  $("a" + eval(div_id - jsonObj.data.length - 2)).style
                    .display != "none"
                )
                  $(
                    "a" + eval(div_id - jsonObj.data.length - 2)
                  ).style.display = "none";
              } catch (e) {}
            }
          });
  ...
  var AjaxReadRequest = function() {
    ...
    if (
      ...
      if (
        !self.inactivity_message_flag &&
        time_now - self.time_last_send > self.interval_for_inactivity - 30000
      ) {
        self.sendSysMessages(
          $("room").value,
          lang_AjaxReadRequest_1,
          self.user_id
        );
        self.inactivity_message_flag = true;
      }
      // Pruefung wie lange der User schweigt und rauswurf
      if (time_now - self.time_last_send > self.interval_for_inactivity)
        location.href =
          "./?Logout&random_user_number=" +
          self.random_user_number +
          "&reason=timeout&r=" +
    ...
    if (time_now - self.time_last_req > eval(self.reload_interval / 2)) {
      ...
      if (!self.active_get_message_req) {
        ...
        var myAjaxObj = new Ajax.Request("./?ReloaderMessages", {
          ...
          postBody:
            ...
            $("room").value +
            "&privat=" +
            encodeURIComponent($("privat").value)
        });
      }
    }
  };
  ...
  this.send = function() {
    ...
    if ($F("privat") > 0 && !self.userOnlineNow($F("privat"))) {
      var win_warning_user_away = new Window({
        className: self.win_style,
        title: lang_warning_user_away_2_1,
        width: 280,
        height: 70,
        resizable: false,
        showEffect: Effect.Appear,
      ...
      win_warning_user_away.setHTMLContent(
        '<div style="padding:3px;"><span><img src="img/messagebox_warning.png" style="padding-right:10px;" width="22" height="22" align="left" />' +
          lang_warning_user_away_2_2 +
          "</span></div>"
      );
      win_warning_user_away.showCenter();
      setTimeout(function() {
        try {
          win_warning_user_away.close();
        } catch (e) {
          /*nix */
        }
      }, 10000);
  ...
  this.send2privatwin = function(message_input_field, privat) {
    ...
    if (!self.userOnlineNow(privat)) {
      ...
      $(win_p).innerHTML +=
        '<div class="privat_ausg_an" style="padding:3px;"><span><b>' +
        lang_warning_user_away_1_1 +
        "</b><br />" +
        lang_warning_user_away_1_2 +
        "</span></div>";
    }
    ...
    var myAjaxObj = new Ajax.Request("./?ReloaderMessages", {
      onSuccess: function(erg) {
        if ($(message_input_field).disabled)
          setTimeout(function() {
  ...
  this.userOnlineNow = function(user_id) {
    for (var i = 0; i < self.jsonObjUserGlobal.userOnline.length; i++)
      if (
        self.jsonObjUserGlobal.userOnline[i].user_id == user_id &&
        self.jsonObjUserGlobal.userOnline[i].user_simg != "status_invisible"
      )
        return true;
  ...
  this.userOnline = function() {
    new Ajax.PeriodicalUpdater("", "./?ReloaderUserOnline", {
  ...
  this.historyWindow = function(seite) {
    ...
    if (typeof self.win_history != "object") {
      ...
      var hoehe = $("chatinhalt").getHeight();
      var breite = $("chatinhalt").getWidth();
      self.win_history = new Window({
        className: self.win_style,
        width: eval(breite - 10),
        title: lang_historyWindow_1,
        height: eval(hoehe - 10),
        top: 20,
        left: 20,
        resizable: true,
        showEffect: Effect.Appear,
      ...
      self.win_history.setHTMLContent(
    ...
    new Ajax.Request("./?History", {
      onSuccess: function(result) {
        ...
        $("site_selecter").onchange = function() {
          self.historyWindow($("site_selecter").value);
        };
        Event.observe("history_seiten", "click", function(event) {
          if (Event.element(event).id.slice(0, 12) == "historysite_") {
            Event.stop(event);
            self.historyWindow(
              Event.element(event).id.slice(12, Event.element(event).id.length)
            );
          }
  ...
  var updateUserOnlineAnzeige = function(ajaxResultJSON) {
    ...
    if (jsonObj) {
      ...
      for (var i = 0; i < jsonObj.userOnline.length; i++) {
        if (jsonObj.userOnline[i].user_id == self.user_id)
          self.userPrivilegienGlobal = jsonObj.userOnline[i].user_priv;
        if (jsonObj.userOnline[i].user_simg != "status_invisible")
          anzahl_der_user_im_chat++;
      }

      var inner_html = "<b>" + lang_updateUserOnlineAnzeige_1 + "</b><br />";
      inner_html +=
        lang_updateUserOnlineAnzeige_2 +
        " " +
        anzahl_der_user_im_chat +
        "<br/><br/>";
      ...
      for (var i = 0; i < jsonObj.userOnline.length; i++) {
        if (aktuelle_room_id != jsonObj.userOnline[i].room_id) {
          ...
          for (var ii = 0; ii < jsonObj.userOnline.length; ii++) {
            if (
              aktuelle_room_id == jsonObj.userOnline[ii].room_id &&
              jsonObj.userOnline[ii].user_simg != "status_invisible"
            )
              anz_im_room++;
          }

          var show_anz_pro_room =
            anz_im_room > 0 ? " (" + anz_im_room + ")" : "";

          var allowed = jsonObj.userOnline[i].room_allowed;
          if (allowed == 1)
            inner_html +=
              '<div class="rooms" id="room_' +
              aktuelle_room_id +
              '">' +
            ...
            if (allowed == 2)
              inner_html +=
                '<div class="rooms_not_allowed" id="pwroom_' +
                aktuelle_room_id +
                '">' +
                jsonObj.userOnline[i].room +
                show_anz_pro_room +
                '<img id="pwroomimg_' +
                aktuelle_room_id +
                '" src="img/keylayer.png" width="11" height="11" style="margin-left:5px;" /></div>';
            else
              inner_html +=
                '<div class="rooms_not_allowed" id="notallowedroom_' +
                aktuelle_room_id +
                '">' +
                jsonObj.userOnline[i].room +
                show_anz_pro_room +
                '<img id="notallowedroomimg_' +
                aktuelle_room_id +
                '" src="img/locklayer.png" width="11" height="10" style="margin-left:5px;" /></div>';
          }
        }
        ...
        var admin_user =
          self.userPrivilegienGlobal == "admin" ||
          self.userPrivilegienGlobal == "mod"
            ? ' <img src="img/wand.png" id="adminu_' +
              jsonObj.userOnline[i].user_id +
              '" /> '
            : "";
        if (
          (self.userPrivilegienGlobal == "admin" &&
            self.user_id == jsonObj.userOnline[i].user_id) ||
          (self.userPrivilegienGlobal == "mod" &&
            self.user_id == jsonObj.userOnline[i].user_id)
        )
          admin_user = ' <img src="img/space.gif" weight="16" height="16" /> ';
        //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        var usr_priv = "";
        if (jsonObj.userOnline[i].user_priv == "admin")
          usr_priv = ' <img src="img/admin_i.png" alt="admin" title="admin">';
        if (jsonObj.userOnline[i].user_priv == "mod")
          usr_priv = ' <img src="img/mod_i.png" alt="mod" title="mod">';
        // -----------------

        //status ----
        if (!jsonObj.userOnline[i].user_simg.empty())
          var user_status_anz =
            ' <img src="img/' +
            jsonObj.userOnline[i].user_simg +
            '.png" alt="' +
            jsonObj.userOnline[i].user_stext +
            '" title="' +
            jsonObj.userOnline[i].user_stext +
            '">';
        ...
        //hide User
        if (
          !jsonObj.userOnline[i].user_simg.empty() &&
          jsonObj.userOnline[i].user_simg == "status_invisible"
        )
          var user_style = "display:none;";
        else var user_style = "";

        // strike user
        if (jsonObj.userOnline[i].user.slice(0, 8) == "<strike>") {
          jsonObj.userOnline[i].user = jsonObj.userOnline[i].user.slice(
            8,
            jsonObj.userOnline[i].user.length
          );
          var user_style2 = "text-decoration: line-through;";
        } else var user_style2 = "";

        if (self.user_id == jsonObj.userOnline[i].user_id) {
          if (jsonObj.userOnline[i].user_sex == "m")
            var gender_icon = "user_comment_m_self.png";
          if (jsonObj.userOnline[i].user_sex == "f")
            var gender_icon = "user_comment_w_self.png";
          if (jsonObj.userOnline[i].user_sex == "n")
            var gender_icon = "user_comment_n_self.png";
          inner_html +=
            '<div id="user_div_' +
            jsonObj.userOnline[i].user_id +
            '" style="' +
          ...
          if (jsonObj.userOnline[i].user_sex == "m")
            var gender_icon = "user_comment_m.png";
          if (jsonObj.userOnline[i].user_sex == "f")
            var gender_icon = "user_comment_w.png";
          if (jsonObj.userOnline[i].user_sex == "n")
            var gender_icon = "user_comment_n.png";
          inner_html +=
            '<div id="user_div_' +
            jsonObj.userOnline[i].user_id +
            '" style="' +
            ...
            lang_updateUserOnlineAnzeige_3 +
            '" alt="' +
            lang_updateUserOnlineAnzeige_3 +
            '" />\
                            <span id="infoblock_' +
        ...
        if (self.user_id == jsonObj.userOnline[i].user_id)
          var aktuell_room = "room_" + aktuelle_room_id;
      }

      try {
        for (var i = 0; i < jsonObj.all_empty_rooms.length; i++)
          if (jsonObj.all_empty_rooms[i].room_allowed == 1)
            inner_html +=
              '<div class="rooms" id="room_' +
              jsonObj.all_empty_rooms[i].room_id +
              '">' +
              jsonObj.all_empty_rooms[i].room +
              "</div>";
          else {
            if (jsonObj.all_empty_rooms[i].room_allowed == 2)
              inner_html +=
                '<div class="rooms_not_allowed" id="pwroom_' +
                jsonObj.all_empty_rooms[i].room_id +
                '">' +
  ...
  var changeUserEvent = function(ereignis) {
    ...
    if (ereignis.slice(0, 7) == "privat_") {
      ...
      // Privat window
      var user_name_p =
        self.privat_an.length > 20
          ? self.privat_an.slice(0, 20) + "..."
          : self.privat_an;

      $("privat_anzeige").innerHTML =
        lang_changeUserEvent_privat_1 +
        " <b>" +
        user_name_p +
        '</b>&nbsp;&nbsp;&nbsp;<span id="close_privat">' +
        lang_changeUserEvent_privat_2 +
        "</span>";
    ...
    if (ereignis.slice(0, 10) == "privatwin_") {
      ...
      if (
        typeof self.win_private[win_id1] != "object" &&
        typeof self.win_private[win_id2] != "object"
      ) {
        var privat_win_opponent = privat2name;
        var privat_win_opponent_id = privat2id;

        self.win_private[win_id1] = new Window({
          className: self.win_style,
          title: "Privat mit " + privat_win_opponent,
          width: 380,
          height: 200,
        ...
        self.win_private[win_id1].setHTMLContent(
          '<div id="pivate_win_' +
            ...
            '" class="privat_mesages_window"></div><div><form style="display:inline" id="win_form_' +
            win_id1 +
            '"><table cellspacing="0" cellpadding="0"><tr><td><input type="text" class="private_message_field" id="message_win_' +
            win_id1 +
            '" ></td><td><img src="img/Checked_small.png" id="submit_img_' +
            win_id1 +
            '" style="padding-left: 4px; cursor:pointer;"></td><td><img src="img/Smiley_small.png" id="smileys_img_' +
        ...
        myObserver = {
          onEndResize: function(eventName, win) {
            if (win == self.win_private[win_id1])
              $("pivate_win_" + win_id1).style.height =
                eval(win.getSize().height - 33) + "px";
          },
          onClose: function(eventName, win) {
            $("message").focus();
          }
        ...
        $("message_win_" + win_id1).onfocus = function() {
          ...
          $("message_win_" + win_id1).style.fontWeight = $(
            "message"
          ).style.fontWeight;
          $("message_win_" + win_id1).style.fontStyle = $(
            "message"
          ).style.fontStyle;
        };
      }
      ...
      var int_id =
        typeof self.win_private[win_id1] == "object" ? win_id1 : win_id2;
      try {
        self.win_private[int_id].show();
      } catch (w) {}
    ...
    if (ereignis.slice(0, 5) == "room_") {
      ...
      for (var i = 0; i < self.jsonObjUserGlobal.userOnline.length; i++)
        if (
          self.jsonObjUserGlobal.userOnline[i].room_id ==
          ereignis.slice(5, ereignis.length)
        ) {
      ...
      try {
        for (var i = 0; i < self.jsonObjUserGlobal.all_empty_rooms.length; i++)
          if (
            self.jsonObjUserGlobal.all_empty_rooms[i].room_id ==
            ereignis.slice(5, ereignis.length)
          ) {
    ...
    if (
      ereignis.slice(0, 10) == "infoblock_" ||
      ereignis.slice(0, 10) == "inflblock_"
    ) {
      ...
      for (var i = 0; i < self.jsonObjUserGlobal.userOnline.length; i++)
        if (
          self.jsonObjUserGlobal.userOnline[i].user_id ==
          ereignis.slice(10, ereignis.length)
        ) {
      ...
      if (typeof self.win_block[id] != "object") {
        ...
        self.win_block[id] = new Window({
          className: self.win_style,
          title: lang_changeUserEvent_infoblock_1 + " " + user_name,
          width: 250,
          height: 90,
          top: pos_top,
          left: pos_left,
          ...
          showEffectOptions: {
            duration: 0.4,
            afterFinish: function(effect) {
              close_info_win(self.win_block[id].getId());
            }
          },
        ...
        var disabled_block =
          user_priv == "admin" || user_priv == "mod" ? "disabled" : "";

        var separate_win_privat_menue = self.allowed_privates_in_separate_win
          ? '<img src="img/privat_win.png" align="left" />&nbsp;&nbsp;<a href="#" id="info_privatf_' +
            id +
            '" >' +
            lang_changeUserEvent_infoblock_4 +
            "</a><br />"
          : "";
        var chat_privat_menue = self.allowed_privates_in_chat_win
          ? '<img src="img/privat_chat.png" align="left" />&nbsp;&nbsp;<a href="#" id="info_privatm_' +
            id +
            '" >' +
            lang_changeUserEvent_infoblock_3 +
            "</a><br />"
          : "";
        var block_option4privat =
          !self.allowed_privates_in_separate_win &&
          !self.allowed_privates_in_chat_win
            ? ""
            : '<input type="Checkbox" id="blockiere_user_priv_' +
              id +
              '" ' +
              disabled_block +
              "> " +
              lang_changeUserEvent_infoblock_7;

        self.win_block[id].setHTMLContent(
          '\
      <img src="img/set_name.png" align="left" />&nbsp;&nbsp;<a href="#" id="info_set_name_' +
            id +
            '" >' +
      ...
      " +
            ...
            '\
      <img src="img/delete.png" align="left" />&nbsp;&nbsp;<a href="#" id="info_blockform_' +
            id +
            '" >' +
            lang_changeUserEvent_infoblock_5 +
            '</a>\
      <div id="block_form_div_' +
            id +
            '" style="display:none;margin-top:2px;"><form name="user_block_' +
            id +
            '" style="display:inline;" >\
      <input type="Checkbox" id="blockiere_user_all_' +
            id +
            '" ' +
            disabled_block +
            "> " +
      ...
      $("info_blockform_" + id).onclick = function() {
        ...
        if (make_ajax_request_of_blocking)
          new Ajax.Request("./?BlockUser", {
            onSuccess: function(result) {
              if (result.responseText == "all")
                $(
                  "blockiere_user_all_" + ereignis.slice(10, ereignis.length)
                ).checked = true;
              else
                $(
                  "blockiere_user_all_" + ereignis.slice(10, ereignis.length)
                ).checked = false;
              if (result.responseText == "priv")
                $(
                  "blockiere_user_priv_" + ereignis.slice(10, ereignis.length)
                ).checked = true;
              else
                $(
                  "blockiere_user_priv_" + ereignis.slice(10, ereignis.length)
      ...
      $("blockiere_user_all_" + id).onclick = function() {
        try {
          $("blockiere_user_priv_" + id).checked = false;
        } catch (e) {}
        ...
        new Ajax.Request("./?BlockUser", {
          onSuccess: function() {
            $("block_form_div_" + id).hide();
            self.win_block[id].close();
            updateUserOnlineAnzeigeAfterRoomChange();
          },
    ...
    if (ereignis.slice(0, 7) == "adminu_") {
      ...
      for (var i = 0; i < self.jsonObjUserGlobal.userOnline.length; i++)
        if (
          self.jsonObjUserGlobal.userOnline[i].user_id ==
          ereignis.slice(7, ereignis.length)
        ) {
      ...
      if (typeof self.win_admin_user[id] != "object") {
        self.win_admin_user[id] = new Window({
          className: self.win_style,
          width: 350,
          height: 180,
          top: eval(self.mouse_top - 10),
          left: eval(self.mouse_left - 390),
          resizable: false,
          showEffect: Effect.Appear,
          hideEffect: Effect.Fade,
          showEffectOptions: { duration: 0.5 },
      ...
      if (user_priv != "admin" && user_priv != "mod")
        self.win_admin_user[id].setHTMLContent(
          "<b>" +
            user_name +
            "</b><br /><i>" +
            user_ip +
        ...
        <option value="0">' +
            lang_changeUserEvent_adminu_opt_1 +
            '</option>\
        <option value="600">' +
            lang_changeUserEvent_adminu_opt_2 +
            '</option>\
        <option value="1800">' +
            lang_changeUserEvent_adminu_opt_3 +
            '</option>\
        <option value="3600">' +
            lang_changeUserEvent_adminu_opt_4 +
            '</option>\
        <option value="10800">' +
            lang_changeUserEvent_adminu_opt_5 +
            '</option>\
        <option value="86400">' +
            lang_changeUserEvent_adminu_opt_6 +
            '</option>\
        <option value="604800">' +
            lang_changeUserEvent_adminu_opt_7 +
            '</option>\
        <option value="94348800">' +
            lang_changeUserEvent_adminu_opt_8 +
            '</option>\
        </select> &nbsp;&nbsp;&nbsp;<a href="#" id="sperre_user_' +
            ereignis.slice(7, ereignis.length) +
            '">' +
        ...
        self.win_admin_user[id].setHTMLContent(
          "<b>" +
            user_name +
            "</b> " +
            lang_changeUserEvent_adminu_3 +
            " " +
            user_priv +
            " " +
            lang_changeUserEvent_adminu_4
        );
      ...
      try {
        $("sperre_user_" + id).onclick = function() {
          new Ajax.Request("./?Insert2Blacklist", {
            onSuccess: function() {
              self.win_admin_user[id].close();
            },
    ...
    //##############################################################################################################
    if (
      ereignis.slice(0, 15) == "notallowedroom_" ||
      ereignis.slice(0, 18) == "notallowedroomimg_"
    ) {
      var win_allowed = new Window({
        className: self.win_style,
        width: 240,
        height: 80,
        top: eval(self.mouse_top - 10),
        left: eval(self.mouse_left - 280),
        resizable: false,
        showEffect: Effect.Appear,
        hideEffect: Effect.Fade,
        showEffectOptions: { duration: 0.5 },
      ...
      win_allowed.setHTMLContent(
        lang_changeUserEvent_notallowedroom_1 +
          '<br><br><div style="width:90px;cursor:pointer;border:1px solid black;text-align:center;" id="close_room_warning">' +
    ...
    if (
      ereignis.slice(0, 7) == "pwroom_" ||
      ereignis.slice(0, 10) == "pwroomimg_"
    ) {
      var room_id_now =
        ereignis.slice(0, 7) == "pwroom_"
          ? ereignis.slice(7, ereignis.length)
      ...
      win_laypw.setHTMLContent(
        '<form id="layerpw_form">' +
          lang_changeUserEvent_pwroom_1 +
          ' <div id="pwroom_error"></div><br /><input type="password" name="layerpw" id="layerpw" style="float:left" />' +
          '<div style="width:60px;cursor:pointer;border:1px solid black;text-align:center;float:left; margin:2px;" id="send_room_pw">' +
          lang_changeUserEvent_notallowedroom_2 +
          "</div></form>"
      );
      win_laypw.show();
    ...
    new Ajax.Request("./?CheckRoomPw", {
      onSuccess: function(result) {
        if (result.responseText == 1) {
          ...
          $("pwroom_error").innerHTML =
            "<b>" + lang_changeUserEvent_pwroom_2 + "</b>";
        }
      },
  ...
  var updateUserOnlineAnzeigeAfterRoomChange = function() {
    new Ajax.Request("./?ReloaderUserOnline", {
      onSuccess: updateUserOnlineAnzeige,
